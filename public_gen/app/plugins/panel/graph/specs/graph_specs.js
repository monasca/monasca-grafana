        ...
        execute: function () {///<reference path="../../../../headers/common.d.ts" />
            common_1.describe('grafanaGraph', function () {
                ...
                graphScenario('when logBase is log 10 and data points contain only zeroes', function (ctx) {
                    ...
                    common_1.it('should not set min and max and should create some fake ticks', function () {
                        var axisAutoscale = ctx.plotOptions.yaxes[0];
                        common_1.expect(axisAutoscale.transform(100)).to.be(2);
                        common_1.expect(axisAutoscale.inverseTransform(-3)).to.be(0.001);
                        common_1.expect(axisAutoscale.min).to.be(undefined);
                        common_1.expect(axisAutoscale.max).to.be(undefined);
                        common_1.expect(axisAutoscale.ticks.length).to.be(2);
                        common_1.expect(axisAutoscale.ticks[0]).to.be(1);
                        common_1.expect(axisAutoscale.ticks[1]).to.be(2);
                        common_1.expect(axisAutoscale.tickDecimals).to.be(undefined);
                    });
                });
                ...
                graphScenario('when logBase is log 10 and y-min is set to 0 and auto min is > 0.1', function (ctx) {
                    ...
                    common_1.it('should set min to 0.1 and add a tick for 0.1 and tickDecimals to be 0', function () {
                        var axisAutoscale = ctx.plotOptions.yaxes[0];
                        common_1.expect(axisAutoscale.transform(100)).to.be(2);
                        common_1.expect(axisAutoscale.inverseTransform(-3)).to.be(0.001);
                        common_1.expect(axisAutoscale.min).to.be(0.1);
                        common_1.expect(axisAutoscale.max).to.be(10000);
                        common_1.expect(axisAutoscale.ticks.length).to.be(6);
                        common_1.expect(axisAutoscale.ticks[0]).to.be(0.1);
                        common_1.expect(axisAutoscale.ticks[5]).to.be(10000);
                        common_1.expect(axisAutoscale.tickDecimals).to.be(0);
                    });
                });
                graphScenario('when logBase is log 2 and y-min is set to 0 and num of ticks exceeds max', function (ctx) {
                    ...
                    common_1.it('should regenerate ticks so that if fits on the y-axis', function () {
                        var axisAutoscale = ctx.plotOptions.yaxes[0];
                        common_1.expect(axisAutoscale.min).to.be(0.1);
                        common_1.expect(axisAutoscale.ticks.length).to.be(8);
                        common_1.expect(axisAutoscale.ticks[0]).to.be(0.1);
                        common_1.expect(axisAutoscale.ticks[7]).to.be(262144);
                        common_1.expect(axisAutoscale.max).to.be(262144);
                        common_1.expect(axisAutoscale.tickDecimals).to.be(0);
                    });
                    common_1.it('should set axis max to be max tick value', function () {
                        common_1.expect(ctx.plotOptions.yaxes[0].max).to.be(262144);
                    });
                });
